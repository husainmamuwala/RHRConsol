...
export default function ContactForm({ }: Props) {
    ...
    const onSubmit: SubmitHandler<Inputs> = formData => {
        window.location.href = `mailto:info@rhrconsol.in?subject=Enquiry&body=
phone Number: ${formData.phone}${formData.message}`;
    };


    return (
        <div className=" bg-white text-black px-10 lg:px-20 pb-10 lg:py-20">
            <h2 className="text-lg lg:text-xl">Contact Form</h2>
            <form
                onSubmit={handleSubmit(onSubmit)}
                id="form"
                className="mt-6 lg:mt-10 flex flex-col gap-y-4"
            >
                ...
                <button
                    className="bg-[#FFCA05] text-black mt-4 lg:mt-2 text-lg w-max px-6 py-2 
