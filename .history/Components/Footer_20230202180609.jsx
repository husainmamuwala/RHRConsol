  ...
  return (
    <div className='bg-[#1C2C5E] text-white py-8 flex md:flex-row flex-col items-center gap-y-6 md:justify-around'>
      ...
      <div className='md:w-2/5'>
        ...
        {
          Links.map((link)=>(
        <div className='flex flex-col text-center gap-y-4 uppercase items-center justify-center text-[#B4ABAB] font-semibold text-sm md:flex-row md:w-2/5'>
        <Link href={link.link} className='hover:text-[#FFCA05] class="w-full md:w-max mx-2 lg:mx-4 xl:mx-6 items-center transition-all'>{link.name}</Link>
        </div>
          ))
        }
      </div>
